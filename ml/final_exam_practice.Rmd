...
Unsupervised learning is about finding patterns in the data. Uncovering patterns in the data can lead to more effective feature engineering. Plus using techniques like PCA can much a model much more efficient! 
...
raw_income = read_csv("openml_1590.csv", na=c("?")) %>%
  drop_na() %>%
  mutate(income_above_50k = as.factor(class==">50K")) %>%
  select(-class) 
...
```{r}
income_rec <-
  recipe(income_above_50k ~ ., data = train) %>% 
  update_role(`native-country`, new_role = "keeper") %>% 
  step_BoxCox(all_numeric()) %>%
  step_dummy(all_nominal(), -all_outcomes(), -`native-country`) 


rf_model = rand_forest(mode = "classification") %>% set_engine('ranger')
...
income_fit1 = income_wflow %>% 
  fit(data = train)

#confusion matrix 
cm1 <- predict(income_fit1, test) %>%
bind_cols(test %>% select(income_above_50k)) %>%
conf_mat(truth = income_above_50k, .pred_class)
...
```{r}
income_fit2 = income_wflow %>% 
  fit(data = downsample_train)

#confusion matrix 
cm2 <- predict(income_fit2, test) %>%
bind_cols(test %>% select(income_above_50k)) %>%
conf_mat(truth = income_above_50k, .pred_class)
...
income_wflow2 = workflow() %>% 
  add_model(lr_model) %>% 
  add_recipe(income_rec) 

income_fit_lr = income_wflow2 %>% 
  fit(data = train)
...
#show the odds ratios for top coefficients
odds_ratio2 <- exp(coef(fit2$finalModel))
data.frame(name = names(odds_ratio2), odds_ratio = odds_ratio2) %>%
arrange(desc(odds_ratio)) %>%
head()


#married-AF-spouse is the top coefficient in the the balanced training model. 
```
```{r}
control <- trainControl(method = "cv", number = 5)
